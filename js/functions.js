...
function set_tipo_pessoa(form,campo,obj){
	...
	if (obj.value == "J"){
		...
		document.getElementById("label_pessoa_fisica_juridica").innerHTML = 'Razão Social';
				  ...
				  if( el[y].id.toString().indexOf("Container") == 0){					  
						...
						if (
							...
							if (
								...
								(parseInt(window.document.body.clientWidth) > 400)  // não deixa distribuir em tablets muito pequenos
								){ 					
									// se tela anterior é menor que x vezes o tamanho do browser maximizado, entao distribui em x vezes
									if(document.getElementById("table_menu_vertical")){  // se está habilitado em menu verticaL								
											if (document.getElementById("table_menu_vertical").clientWidth == 22){ // se menu vertical está fechado
...
function distribuir_responsivo(obj,add_width,add_height){  // uma só coluna

			
			if(document.getElementById("table_menu_vertical")){  // se está habilitado em menu verticaL
				 if (document.getElementById("table_menu_vertical").clientWidth == 22){ // se menu vertical está fechado
				 	 var leftposition = 22;
				 }else{  // se menu vertical está aberto
...
function openAeroWindow(div,PositionTop,PositionLeft,Width,Height,Title,iframe_src){				
		   ...
		   // verifica se janela é maior que o tamanho do browser
...
function inserir_contribuicao(form){	  		
	...
	var msg= new Array("Título da Execução","Escolher Tipo Media","Desafio","Membro");		
	for (x=0;x<vetor.length;x++){	 	
		...
		$( "#menu_lateral" ).click(function() {							
			if (document.getElementById("table_menu_vertical").style.width == '22px')  {				
				 ...
				 var y = setTimeout(function() { 									  
				  ...
				  //  cascade_windows();  não pode executar pois menu está andando em tempo real
			}			
...
function is_jpg(str){	
	if (str){
		...
		if(arraylist[arraylist.length-1].toLowerCase() != "jpg"){				
			  var ok = confirm("Sua Assinatura não está no formato JPG e por isso não será importada. Deseja continuar?");
			  if(ok){	
				return true;						
			  }else{
				return false;	  
			  }
			alert("Sua não está no formato JPG");
		}else{
				 ...
				 if (campo_not_null[y] == input[x].name){
						...
						switch (field_type){
							  ...
							  case "textarea":					
								...
								for (i=0; i < self.frames.length; i++){
										if(document.getElementById(id_xinha) != null){ //se textarea está habilitado com xinha
											  if (self.frames[i].name.search("Xinha")!=-1){	
...
function alterar_grupo_acesso(form,cod){	
	...
	if (!checado){		
		 var ok = confirm("Nenhum checkbox foi marcado, essa alteração removerá todos os privilégios desse usuário. Confirma?");
		  if(ok){
			  //segue algoritmo
		  }else{
...
function adicionar_grupo_acesso(form){
	...
	if (!checado){		
		 var ok = confirm("Nenhum checkbox foi marcado, usuários desses grupo terão todos os privilégios removidos. Confirma?");
		  if(ok){
			  //segue algoritmo
		  }else{
			    $('#tabs').tabs('select', 'tabs-controle');
	...
	if (newDataValidate(obj)){		
			...
			setTimeout(function() {   						
				  ...
				  if(subtrai_data(obj.value,data_fim) < 0){					
					  showWarningToast("Sua Data não pode ser maior que  "+ data_fim +" !");	
					  obj.value = "";					  
					  return false;
				  }		
				  ...
				  if(subtrai_data(obj.value,data_inicial) > 0){					
					  showWarningToast("Sua Data não pode ser menor que  "+ data_inicial +" !");	
					  obj.value = "";					  
					  return false;
				  }		
			}, 500);

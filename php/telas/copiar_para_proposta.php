...
if ($_GET["id_contribuicao"]){  // Alterar desafio
	$sql_check = "SELECT id_contribuicao from contribuicoes
			WHERE (id_contribuicao_origem = '".$_GET["id_contribuicao"]."')  AND id_fase = '2'
			limit 1
			";  
	if (numrows($sql_check)>0){
		echo "Inspiração já copiada anteriormente";	
	}else{			
...
SELECT  NULL,  nome_contribuicao,  aprovado,  'N' as selecionado,  media_flag,  youtube_link,  caminho_arquivo,  arquivo,  id_participante, '2' as id_fase,  id_desafio,  id_topico,  descricao,  motivo_reprovacao, '".$_GET["id_contribuicao"]."' as id_contribuicao_origem, '".date("Y-m-d H:i:s")."' as  data_inclusao FROM contribuicoes WHERE (id_contribuicao = '".$_GET["id_contribuicao"]."')  AND id_fase = '1' LIMIT 1";
		insert_record($sql);
		echo "Inspiração copiada para Proposta";	
	}		
}

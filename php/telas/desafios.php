...
if ($_GET["cod"]){  // Alterar desafio
	$sql = "SELECT id_desafio,nome_desafio,descricao,data_fim,data_inicio,media_flag
			FROM desafios as s
			WHERE (s.id_desafio = '".$_GET["cod"]."') 
			limit 1
			";  	
	$rs = get_record($sql);		
	...
	$title_desafio = $rs["nome_desafio"];	
}else{  //Adicionar Nova entidade
	$title_desafio = "Incluir Desafio";
}
...
<input type="hidden" value="<?= $_GET["cod"] ?>" name="id_desafio" id="id_desafio">

<? create_table_extjs("99%","130px",$title_desafio);?>

<? require("form_desafio.php"); ?>
  <tr nowrap>
   <td align="left">
...
function atualiza_arquivos(){
	atualiza_conteudo("operacoes.php","id_desafio=<?= $_GET["cod"]  ?>&listar_arquivos=yes","POST",handleHttpArquivos);
}

...
if ($_GET["id_contribuicao"]){  // Alterar desafio
	$sql= "SELECT id_contribuicao,nome_contribuicao, media_flag,  youtube_link,  caminho_arquivo,  arquivo,    id_fase,  id_desafio,  descricao  from contribuicoes
			WHERE (id_contribuicao = '".$_GET["id_contribuicao"]."')  AND id_fase = '4'
			limit 1
			";  
...
<? require("form_contribuicao.php"); ?>
 ...
 <td colspan="5" align="right">
          <input type="button" name="btn_salvar" value="Enviar" onclick="inserir_contribuicao(this.form)" style="margin-right:5px;"   class="botao" onMouseOver="this.className='botao_hover'" onMouseOut="this.className='botao'"   >          
...
function adicionar_video(form) {
	...
	for (x=0;x<vetor.length;x++){	
	  ...
	  if  ( (form.myYouTubePlayer.value.match(p)) ? RegExp.$1 : false ){		
			...
			form.action = "operacoes.php?adicionar_youtube=yes"+"&value="+ encodeURIComponent(RegExp.$1)+"&time="+myDate.getTime();
			form.submit();

...
FROM desafios as a
WHERE a.data_cancelamento is NULL AND a.data_inicio < now() and id_desafio = '".$id_desafio."';
";
 ...
 ?>
  ...
  <div style="margin:0 auto;margin-top:0px;background-color:transparent;width:980px;height:auto;" >
    <table cellpadding="0" cellspacing="0">
	  ...
	  <?				  
        $fases = explode("|",show_menu_fases($rs_desafio["id_desafio"],$id_fase,'1'));  //motra menu de fases e pega a fase atual do desafio	
		$fase_atual = $fases[0];
		$nome_fase = $fases[1];
		
        desafio_info($rs_desafio["media_flag"],$caminho,$arquivo,$youtube,$rs_desafio["descricao"],$descricao_anexo,$rs_desafio["id_desafio"],$id_fase,$nome_fase,'1','0');
      ...
      <div style="width:980px;height:auto;">
        <center>
	  <?
	  echo '<div style="width:940px;height:auto;border-top:1px #ccc solid"></div>';
	 // echo show_form_inspiracao($rs_desafio["id_desafio"],$fase_atual);
	 
	 filtro_contribuicao($id_fase,$rs_desafio["id_desafio"]);
	 
	  show_contribuicao($rs_desafio["id_desafio"],$id_fase,'1');   
